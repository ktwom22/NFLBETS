...
.flip-card-inner { position: relative; width: 100%; height: 250px; text-align: center; transition: transform 0.6s; transform-style: preserve-3d; box-shadow: 0 4px 8px rgba(0,0,0,0.2); border-radius: 15px; }
.flip-card.flip .flip-card-inner { transform: rotateY(180deg); }
.flip-card-front, .flip-card-back { position: absolute; width: 100%; height: 100%; backface-visibility: hidden; border-radius: 15px; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 10px; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    cards.forEach(card => { card.addEventListener('click', function() { card.classList.toggle('flip'); }); });
});
</script>
</head>
<body>
...
{% if game['Away Logo'] %}<img src="{{ game['Away Logo'] }}" class="team-logo"><br>{% endif %}
<strong>{{ game['Away Team'] }}</strong><br>
{% if game['Away Final Points'] %}<span>{{ game['Away Final Points'] }}</span>{% endif %}
{% if game['My Pick'] == game['Away Team'] %}<div class="my-pick-text">My Pick</div>{% endif %}
</div>
<div class="col text-center"><h4>at</h4></div>
<div class="col text-center">
{% if game['Home Logo'] %}<img src="{{ game['Home Logo'] }}" class="team-logo"><br>{% endif %}
<strong>{{ game['Home Team'] }}</strong><br>
{% if game['Home Final Points'] %}<span>{{ game['Home Final Points'] }}</span>{% endif %}
{% if game['My Pick'] == game['Home Team'] %}<div class="my-pick-text">My Pick</div>{% endif %}
</div>
</div>
{% if game['My Pick'] %}<hr><p>Predicted Winner: <strong>{{ game['My Pick'] }}</strong></p>{% endif %}
</div>
<div class="flip-card-back">
{% if game['Rationale'] %}<p>{{ game['Rationale'] }}</p>{% else %}<p>No rationale provided.</p>{% endif %}
</div>
</div>
</div>
</div>
